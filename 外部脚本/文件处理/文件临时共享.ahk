...
SplitPath, HttpServer_File, HttpServer_FileName
...
<p>文件下载:</p>
<p> <a href="/%HttpServer_FileName%">文件共享(%HttpServer_FileName%)</a></p>
<p> <a href="/download">文件共享(固定的下载地址)</a></p>
<p> &nbsp; </p>
...
paths["/download"] := Func("Func_downFile")
paths["/" HttpServer_FileName] := Func("Func_downFileName")

paths["404"] := Func("NotFound")

global server := new HttpServer()
server.LoadMimes(A_ScriptDir . "/../../配置文件/mime.types")
server.SetPaths(paths)
server.Serve(5566)   ; 端口可能需要更改,  部分端口在某些手机上显示为不安全的端口导致无法访问
...
Gui,66:Default
Gui,66:Destroy
UrL := "http://" A_IPAddress1 ":5566/download"
GUI, Add, Pic, x20 y20 w500 h-1 hwndhimage, % f:=GEN_QR_CODE(UrL)
GUI, Add, Text, x20 y540 h24, 下载地址:
GUI, Add, Edit, xp+60 yp w445 h24, % UrL
GUI, Add, Text, x20 yp+30 h24, 下载网页:
GUI, Add, Edit, xp+60 yp w445 h24, % "http://" A_IPAddress1 ":5566"
GUI, Show, w540 h610
return
...
favicon(ByRef req, ByRef res, ByRef server) {
    server.ServeFile(res, A_ScriptDir . "/../../脚本图标/e1cb.ico")
    res.status := 200
}

NotFound(ByRef req, ByRef res) {
    res.SetBodyText("Page not found! 404!")
    res.status := 404
}
...
Func_downFile(ByRef req, ByRef res) {
    if (!HttpServer_File) {
        ...
        res.SetBodyText("Select the file on the PC first.")
        res.status := 404
        return
    }
...
Func_downFileName(ByRef req, ByRef res) {
    erver.AddHeader(res, "Content-Disposition", "attachment; filename=" HttpServer_FileName)
    server.ServeFile(res, HttpServer_File)
    res.status := 200
}
...
/*! TheGood
    ...
    https://www.autohotkey.com/board/topic/53827-ahksock-a-simple-ahk-implementation-of-winsock-tcpip/
    https://www.autohotkey.com/boards/viewtopic.php?t=15613

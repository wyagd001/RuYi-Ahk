...
myfolderpath := CandySel

if (StrLen(myfolderpath) < 4)
	myfolderpath := A_ScriptDir
...
Gui, Add, ListView, x170 y165 w900 h445 -Readonly Grid Checked hwndHLV vMyListView, 序号`n文件名`n扩展名`n修改日期`n相对路径`n新文件名
gui, add, Button, xp+170 yp+455 w80 gcheckallfile default, 全选
gui, add, Button, xp+90 yp w80 guncheckallfile, 全不选
gui, add, Button, xp+90 yp w80 gcheckallSeletedfile, 选中高亮
gui, add, Button, xp+90 yp w80 guncheckallSeletedfile, 高亮不选
gui, add, Button, xp+90 yp w80 gmodify, 手动编辑

gui, add, Button, xp+90 yp w80 grunrename, 执行

Gui, Add, StatusBar
SB_SetParts(170, 470)
SetTimer 延时加载界面, -300

ICELV := New LV_InCellEdit(HLV)
ICELV.SetColumns(6)

gui, show, AutoSize, 文件批量重命名
return
...
Loop, Files, %myfolderpath%\*.*, FR
{
	...
	R_index++
	LV_Add("", R_index, A_LoopFileName, A_LoopFileExt, A_LoopFileTimeModified, StrReplace(StrReplace(A_LoopFilePath, myfolderpath), A_LoopFileName))
}
GuiControl, +Redraw, MyListView 
LV_ModifyCol()
...
{
	commmode("属性标签的模板:",,, "enable")
	GuiControl,, myedit1, `n\参与创作的艺术家\ - \标题\❤❤❤音频文件`n桂林旅游_\分辨率\❤❤❤图片文件`n拍摄_\帧宽度\*\帧高度\❤❤❤视频文件`n\产品名称\_\文件版本\❤❤❤程序文件
}
else if (comm = "按位置删除")
{
...
else if (comm = "序号")
{
	...
	GuiControl,, myedit1, `n❤❤❤起始编号为1并放在开头`n`n10❤❤❤起始编号为10`n1|02❤❤❤序号长度为2位数`n10|03❤❤❤起始编号为10, 序号长度为3位数`n1|1|前❤❤❤序号放在文件名开头`n1|1|后❤❤❤序号放在文件名结尾`n1|1|前|图片❤❤❤序号放在文件名开头且文件名改为图片`n清除开头数字序号❤❤❤清除开头的数字序号`n清除结尾数字序号❤❤❤清除结尾的数字序号`
}
else if (comm = "按符号分割后重排")
{
	commmode("分割符", "重排顺序:",, "enable", "enable")
	GuiControl,, myedit1, `n-`n`n_`n `n
	GuiControl,, myedit2, \2\ - \1\`n`n\3\ - \1\ - \2\
}
else if (comm = "删除数字或字母")
{
	commmode("删除模式:",,, "enable")
	GuiControl,, myedit1, `n删除数字`n`n删除英文字母`n删除数字和字母`n
}
else if (comm = "简体与繁体")
{
...
ControlGet, newname, List, col6, SysListView321

Gui +Disabled 
Gui, MyeditGui: default
Gui, MyeditGui: +Owner%GuiID%

Gui, Add, Edit, x10 y10 voldnameEdit w300 h300 ReadOnly -Wrap, %oldname%
Gui, Add, Edit, xp+310  vnewnameEdit w300 h300 -Wrap vmodifiednewname -WantReturn, %newname%
...
sumbitlistview:
Gui, MyeditGui: Submit
WinActivate, ahk_id %GuiID%
Gui, 1: Default
Gui -Disabled
Loop, Parse, modifiednewname, `n
{
...
Loop
{
	...
	if not RowNumber  ; 上面返回零, 所以选择的行已经都找到了.
		break
	LV_GetText(oldname, RowNumber, 2)
	LV_GetText(relpath, RowNumber, 5)
	LV_GetText(newname, RowNumber, 6)
	if !newname
		Continue
...
Loop % LV_GetCount()
{
	...
	LV_GetText(A_LvRelPath, A_index, 5)
  LV_GetText(A_LvNewName, A_index, 6)
	...
	if InStr(myedit2, "\")
	{
		...
		newname := SafeFileName(StrReplace(A_LvNewName?A_LvNewName:A_LvFileName, myedit1, ModeS2, OutputVarCount))
	}
	else
		newname := SafeFileName(StrReplace(A_LvNewName?A_LvNewName:A_LvFileName, myedit1, myedit2, OutputVarCount))
	if OutputVarCount
	{
    ;msgbox % A_LvNewName?A_LvNewName:A_LvFileName
		LV_Modify(A_index, "Check col6", newname)
	}
}
LV_ModifyCol()
return
...
Loop % LV_GetCount()
{
  ...
  LV_GetText(A_LvNewName, A_index, 6)
	...
	newname := SafeFileName(RegExReplace(A_LvNewName?A_LvNewName:A_LvFileName, myedit1, myedit2, OutputVarCount))
...
if !RegExMatch(myedit3, "(\d)+\|")
{
    ...
    LV_GetText(A_LvNewName, A_index, 6)
		A_LvFilePath := myfolderpath A_LvRelPath A_LvFileName
    ;tooltip  % A_LvFilePath
		SplitPath, % A_LvNewName?A_LvNewName:A_LvFileName,,, A_LvFileExt, A_LvFileNameNoext
		...
		if InStr(myedit1, "\")
		{
			...
			{
				ModeS1 := StrReplace(ModeS1, "\" v "\", xpro_obj1[v])
			}
		}
		if InStr(myedit2, "\")
		{
		...
		LV_Modify(A_index, "Check col6", newname "." A_LvFileExt)
	}
}
Else
{
...
Else
{
	if !myedit1
	{
      ...
      LV_GetText(A_LvNewName, A_index, 6)
			...
			newname := SafeFileName(myedit2 A_index myedit3 (A_LvNewName?A_LvNewName:A_LvFileName))
			LV_Modify(A_index, "Check col6", newname)
		}
	}
	Else
	{
		; 1|1|前|图片
		MEArr := StrSplit(myedit1, "|")
		StartIndex := MEArr[1] ? MEArr[1] - 1 : 0
		Indexwidth := MEArr[2] ? MEArr[2] : 1
		IndexPos   := MEArr[3] ? (MEArr[3] = "前") ? 1 : 2 : 1
      ...
      LV_GetText(A_LvNewName, A_index, 6)
			...
			if (IndexPos = 1)
			{
				...
				Else
					newname := SafeFileName(myedit2 Format("{:" Indexwidth "}", StartIndex+A_index) myedit3 A_LvNewName?A_LvNewName:A_LvFileName)
			}
			if (IndexPos = 2)
			{
				if specifyName
...
Loop % LV_GetCount()
{
  ...
  LV_GetText(A_LvNewName, A_index, 6)
	...
	if InStr(A_LvNewName?A_LvNewName:A_LvFileName, myedit1)
	{
		ModeS := myedit2
...
Loop % LV_GetCount()
{
  ...
  LV_GetText(A_LvNewName, A_index, 6)
	...
	if (A_LvFileExt = myedit1)
		LV_Modify(A_index, "Check col6", A_LvFileNameNoext "." myedit2)
}
LV_ModifyCol()
Return
...
Kind:
可选的。文件的“种类”。例如: 音乐, 图片, 视频等。该函数将在尝试检索属性之前测试 Kind。
...
*p: 可变参数。以字符串形式传递属性名。我能够传递1317元素线性数组作为参数。
如果最后一个可变参数为xInfo，该函数将返回额外的文件属性，如 A_LoopFileExt, A_LoopFileFullPath 等
*/
; v.90 By SKAN on D1CC @ goo.gl/jyXFo9
...
While ( i++ < nParams )
{
	...
	If ( PropNum := xDetails[Prop] ) > -1
	{
		...
		Else
				fex[Prop] := ObjDir.GetDetailsOf(objItm, PropNum)  ; Folder.GetDetailsOf
		Continue 
	}

	; ShellFolderItem.ExtendedProperty
	; System.Video.FrameWidth          时间会与当地时间差8个小时
...
{
	Arr := []
	str := StrReplace(str, "\", "\", OutputVarCount)
	if (OutputVarCount = 6)
	{
		RegExMatch(str, "\\(.+?)\\.*\\(.+?)\\.*\\(.+?)\\", out)
		Arr[1] := out1, Arr[2] := out2, Arr[3] := out3
		return Arr
	}
	if (OutputVarCount = 4)
	{

...
Gui, Add, ListView, x170 y165 w900 h445 Checked, 文件名`n相对路径`n新文件名
gui, add, Button, xp+170 yp+455 w80 gruncomm default, 全选
gui, add, Button, xp+90 yp w80 gruncomm , 全不选
...
{
	LV_Add("", A_LoopFileName, StrReplace(StrReplace(A_LoopFilePath, myfolderpath), A_LoopFileName))
}
LV_ModifyCol()
modified := 0
return
...
modify:
Gui, 1: Default
ControlGet, oldname, List, col1, SysListView321, ahk_id %GuiID%
ControlGet, newname, List, col3, SysListView321
Gui +Disabled 
...
Loop
{
	...
	if not RowNumber  ; 上面返回零, 所以选择的行已经都找到了.
		break
	LV_GetText(oldname, RowNumber, 1)
...
Loop, Files, %myfolderpath%\*.*, FR
{
	...
	if OutputVarCount
		LV_Add("Check", A_LoopFileName, StrReplace(StrReplace(A_LoopFilePath, myfolderpath), A_LoopFileName), newname)
}
LV_ModifyCol()
return

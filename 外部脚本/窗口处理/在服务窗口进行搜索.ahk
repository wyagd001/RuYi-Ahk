;|2.7|2024.05.23|1607
; 来源网址: https://www.autohotkey.com/boards/viewtopic.php?p=426511#p426511
; 仅在 64 位系统测试过
OnExit, QUIT
WINTITLE = 服务 ahk_class MMCMainFrame
if !winexist(WINTITLE)
	exitapp
objLV := ExtListView_Initialize(WINTITLE)
Allobj := ExtListView_GetAllItems(objLV)
rows := 15
Gui Add, Edit, w500 vfind 
Gui Add, Button, x+10 default gSearch, 搜索
Gui Add, ListView, % "xm vLV1 w550 r" rows " Section glv", % "序号|服务名|服务显示名称|描述"
gui, show,, 搜索服务
Return
...
for k, v in Allobj
{
	loop, 2
	{
		if instr(v[A_index], find)
		{
			LV_Add(, k-1, _GetName_(v[1]), v[1], v[2])
			break
		}
		if (A_index = 1)
		{
			if instr(_GetName_(v[1]), find)
			{
				LV_Add(, k-1, _GetName_(v[1]), v[1], v[2])
				break
			}
...
if (A_GuiEvent = "DoubleClick")
{
	...
	;tooltip % RowNum
	ExtListView_EnsureVisible(objLV, RowNum)
	ExtListView_ToggleSelection(objLV, 1, RowNum)
}
return
...
{ 
	...
	SCM_HANDLE := OpenSCManager(0x1)  ; SC_MANAGER_CONNECT (0x0001)    
	GetServiceKeyName(SCM_HANDLE, DisplayName, 0, Len)
	VarSetCapacity(Buffer, Len * 2) ;Prepare Buffer
	h := GetServiceKeyName(SCM_HANDLE, DisplayName, Buffer, Len * 2)
...
GetServiceKeyName(hService, ServiceDisplayName, ByRef ServiceName, ByRef BufferSize)
{
	return DllCall("advapi32\GetServiceKeyName", "Uint", hService, "Str", ServiceDisplayName, "Uint", &ServiceName, "UintP", BufferSize)
}
...
ExtListView_GetAllItems(ByRef objLV, sState:=0x0000) {
    Try {
        ...
        {
							;tooltip % nRow
            If ( (nRow := ExtListView_GetNextItem(objLV, nRow, sState)) == -1 ) or (nRow == 0xFFFFFFFF)
						{
							;tooltip % nRow
              Break
						}
...
ExtListView_GetItemText(ByRef objLV, nRow, nCol) {
    ...
    ;msgbox % StrGet(&cRecvBuf, objLV.szreadbuf, objLV.senc)
...
ExtListView_DeInitialize(ByRef objLV) {
    ...
    __ExtListView_DeAllocateMemory(objLV)

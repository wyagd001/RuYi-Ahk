...
SJ_ItemsStr := "1. 判断季度||2. 距离合同到期还剩多少天|3. 生肖|4. 天数转为日期(以1900/1/1为起点)|5. 天数转为日期2(Date)|6. 日期转为距1900/1/1的天数(Value)|7. 计算退休日期|8. 计算退休日期|9. 计算日期是一年中的第几周|10.计算是一年中的第几天|11.计算本月的最后一天的日期|12.计算本月的第一天的日期|13.根据日期计算周几"
...
CF_ItemsStr := "1. 是否有重复(满足条件的单元格计数)||2. 重复项目计数(满足多条件的单元格计数)|3. 提取唯一值(重复的只提取一次)|4. 提取唯一值(重复的不提取)|5. 不重复项目计数(多个相同重复的只计为 1)|6. 是否有重复(重复内容首次出现时不提示)|7. 列查找提取同行中的列数据(VLookUp)|8. 反向列查找|9. 非空单元格计数(包含隐藏单元格)|10.非空单元格计数(不包含隐藏单元格)|11.非空单元格计数|12.空单元格计数"
...
, "=IF(COUNTIF(A$1:A$10,A1)>1,""有重复"","""")"
, "=IFNA(VLOOKUP(A1,'[文件名.xlsx]Sheet1'!B$1:B$10,3,0),"""")"
...
CGYS_ItemsStr := "1. 求和||2. 平均值|3. 取整|4. 绝对值|5. 四舍五入|6. 根据身份证号计算退休日期(55岁)|7. 计算退休日期(58, 63岁)|8. |9. |10. |11. "
CGYS_Arr := ["=SUM(A1:A10)"
, "=Average(A1:A10)"
, "=Int(A1)"
, "=ABS(A1)"
, "=Round(A1,2)"
, "=EDATE((MID(A1,7,4)&""/""&MID(A1,11,2)&""/""&MID(A1,13,2)),(60-((IF(MOD(MID(A1,17,1),2)=1,0,1))+1*(IF(MOD(MID(A1,17,1),2)=1,0,1)))*5)*12+MIN(CEILING(MAX(12*(MID(A1,7,4)-1964-((IF(MOD(MID(A1,17,1),2)=1,0,1))+1*(IF(MOD(MID(A1,17,1),2)=1,0,1)))*5)+MID(A1,11,2)-12,0)/IF(AND(IF(MOD(MID(A1,17,1),2)=1,0,1),1),2,4),1),36+1*(IF(MOD(MID(A1,17,1),2)=1,0,1))*24))"
, "=EDATE((MID(A1,7,4)&""/""&MID(A1,11,2)&""/""&MID(A1,13,2)),(60-((IF(MOD(MID(A1,17,1),2)=1,0,1))+0*(IF(MOD(MID(A1,17,1),2)=1,0,1)))*5)*12+MIN(CEILING(MAX(12*(MID(A1,7,4)-1964-((IF(MOD(MID(A1,17,1),2)=1,0,1))+0*(IF(MOD(MID(A1,17,1),2)=1,0,1)))*5)+MID(A1,11,2)-12,0)/IF(AND(IF(MOD(MID(A1,17,1),2)=1,0,1),0),2,4),1),36+0*(IF(MOD(MID(A1,17,1),2)=1,0,1))*24))"
, "="
, "="
, "="]
...
else if Instr(FormulaStr, "WEEKDAY")
	SB_SetText("WEEKDAY(数值,参数), 参数值为2表示星期一为1, 星期天为7")

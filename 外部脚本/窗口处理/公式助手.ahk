...
Gui, Add, ComboBox, xp+70 yp w250 vFormulaTypeIndex gshowNameIndex AltSubmit, 常用||身份证|时间|重复
...
CF_ItemsStr := "1. 是否有重复(满足条件的单元格计数)|2. 重复项目计数(满足多条件的单元格计数)|3. 提取唯一值(重复的只提取一次)|4. 提取唯一值(重复的不提取)|5. 是否有重复(重复内容首次出现时不提示)|6. 列查找提取同行中的列数据|7. 反向列查找|8. 非空单元格计数(包含隐藏单元格)|9. 非空单元格计数(不包含隐藏单元格)"

...
gui, add, Text, xp-140 yp+40 w120 vmyparam2, 参数2:
gui, add, ComboBox, xp+140 yp w450 vmyedit2,
gui, add, Text, xp-140 yp+40 w120 vmyparam3, 参数3:
...
; 查找替换|高亮查找|提取查找字符所在行|移除查找字符所在行|单行正则提取|单行正则替换|提取中文|删除首尾空格制表符|行首尾添加字符|行首添加序号|按数量提取行前后字符|按数量删除行前后字符|按数量分割单行|按行号提取|删除空白行|删除重复行|行倒序排列|行字顺序颠倒|字符串反转|竖排文字|字数统计|查看中文所属字集|左右对比|Base64加密|Base64解密|Unicode转中文|URL解码|首字母转大写|英文字母大小写|中英文添加空格|英文标点改中文|中文标点改英文|全角空格改半角|
updateparam:
Gui Submit, nohide
if (comm = "行首尾添加字符")
{
...
else if (comm = "行首添加序号")
{
	...
	GuiControl,, myedit1, `n12345`n`n一二三四五`n①②③④⑤`n❶❷❸❹❺`n⑴⑵⑶⑷⑸`nabcde(小写)`nABCDE`n
}
else if (comm = "查找替换")
{
...
{
	commmode("连接的字符:",,, "enable")
}
else if (comm = "按行号提取")
{
	commmode("行号:",,, "enable")
	GuiControl,, myedit1, `n奇数`n`n偶数`n1,2,3,7,8,9
}
...
else if (comm = "删除首尾空格制表符")
{
	commmode("模式:",,, "enable")
	GuiControl,, myedit1, `n删除首尾空格制表符`n`n删除行首的空格制表符`n删除行尾的空格制表符
}
else if (comm = "中文转拼音")
{
	commmode("转换模式:",,, "enable")
...
{
	commmode("指定编码:",,, "enable")
	GuiControl,, myedit1, `ncp0❤❤❤ASNI`n`ncp936`ncp950`ncp65001❤❤❤UTF-8`ncp1200❤❤❤UTF-16 LE`ncp1201❤❤❤UTF-16 BE
}
else
{
	commmode()
...
if (myedit1 = 12345)
{
	Loop, Parse, oldtxt, `r, `n
	{
		newStr .= myedit2 A_index myedit3 A_LoopField "`n"
	}
}
else
{
...
Loop, Parse, oldtxt, `r, `n
{
	zh_Str := GetAllCNString(A_LoopField, myedit1)
	if zh_Str
		newStr .= zh_Str "`n"
}
t2.SetText(newStr)
...
Loop, Parse, oldtxt, `r, `n
{
	if (myedit1="删除行首的空格制表符")
		newStr .= LTrim(A_LoopField, "　 `t")  "`n"
	Else if (myedit1="删除行尾的空格制表符")
		newStr .= RTrim(A_LoopField, "　 `t")  "`n"
	Else
		newStr .= Trim(A_LoopField, "　 `t")  "`n"
}
t2.SetText(newStr)
return
...
Loop, Parse, oldtxt, `r, `n
{
	if A_LoopField && Trim(A_LoopField, " `t")
	{

...
### <span id="3.0">3.0 - 2025/08/15</span>
1. 修改 自动运行动作定时动作
.01 新增 定时动作支持指定 某月某日, 每月某日, 每月月底这三种定时类型  
2. 修改 配置文件  
.01 新增 面板列表配置文件 可自定义面板配置文件的路径或文件名  
.02 修改 托盘图标 配置支持 字符编码★字体颜色@字体背景色☆字体名称 的形式显示字体中的字符作为图标  
.03 新增 深色托盘图标 现在可自定义两种模式下的图标, 配置支持 字符编码★字体颜色@字体背景色☆字体名称 的形式显示字体中的字符作为图标  
.04 其他程序中 新增变量项 githubproxy 代理组  
3. 修改 动作管理窗口  
...
.02 新增 外部脚本共享库 /lib/CF.ahk 和 /lib/RuYi.ahk  
6. 新增 外部脚本动作  
.01 新增 粘贴到选中文件.ahk  
.02 新增 文件_gps.ahk  
.03 新增 Winmerge.ahk  

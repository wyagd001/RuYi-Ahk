...
### <span id="3.0">3.0 - 2025/09/07</span>
1. 修改 自动运行动作  
.01 新增 定时动作中支持指定 某月某日, 每月某日, 每月月底这三种定时类型  
.02 新增 监控进程中新增参数 3/4, 每 10 分钟判断指定进程是否存在, 如果存在设置变量 Windy_CurWin_Pid 的值为匹配进程的pid  
2. 修改 配置文件  
.01 内置动作.ini 破坏性修改 以动作编号作为段名, 动作的所有配置项目都写在各自的段中(名称, 图标, 说明, 动作内容, 热键, 鼠标手势等), 取消了 动作热键.ini 和 动作手势.ini 两个配置文件.  
.02 自定义动作.ini 破坏性修改 修改同上. 可使用动作 1732 将旧版的 自定义动作.ini 转为新版配置样式.  
.03 如一.ini 新增 面板列表配置文件 可自定义面板配置文件的路径或文件名  
.04 如一.ini 修改 托盘图标 配置支持 字符编码★字体颜色@字体背景色☆字体名称 的形式显示字体中的字符作为图标  
.05 如一.ini 新增 深色托盘图标 现在可自定义两种模式下的图标, 配置支持 字符编码★字体颜色@字体背景色☆字体名称 的形式显示字体中的字符作为图标  
.06 其他程序.ini 新增 变量项 githubproxy 代理组  
3. 修改 动作管理窗口  
.01 修改 按钮动作图标设置更改为 字符编码★字体颜色@字体背景色☆字体名称@字体大小  
.02 新增 动作图标编辑器 启动按钮, 方便设置动作的图标  
4. 新增 资源管理器右键菜单支持添加文件夹背景菜单  
5. 新增 额外任务栏在左或右边时, 额外任务栏第一个按钮为上次运行的动作  
6. 修改 面板  
.01 面板上新增置顶按钮, 置顶时不会自动关闭面板  
.02 新增 主面板判断支持额外的窗口标题文字匹配, 例如 窗口进程名@部分窗口标题文字 或 窗口类@标题文本, 示例 面板名称为 "notepad@重要" 的面板在窗口为记事本并且标题中有 "重要" 两字时作为主面板显示  
7. 新增 如意内置函数和标签  
.01 新增 内部标签 GenerateWebActionsPage 自动生成动作网页  
.02 新增 内部函数 CF_ScriptExit 指定脚本名称退出指定 ahk 脚本  
8. 新增 环境变量 A_RuYiDir 值为 %A_ScriptDir%, 程序运行时自动写入注册表, 可在外部脚本中使用该系统变量代表如意主目录  
9. 新增 外部脚本  
.01 新增 外部脚本共享库 /lib/CF.ahk  
.02 新增 外部脚本共享库 /lib/RuYi.ahk  
.03 新增 外部脚本共享库 File_CpTransform.ahk  
.04 新增 外部脚本共享库 AutoXYWH.ahk  
.05 新增 外部脚本共享库 OpenedFolder.ahk  
.06 新增 粘贴到选中文件.ahk  
.07 新增 文件_gps.ahk  
.08 新增 Winmerge.ahk  
.09 新增 文件夹_局域网共享(Net Share).ahk  
.10 新增 文件夹_删除局域网共享.ahk  
.11 新增 文件夹_局域网共享(API).ahk  
.12 新增 文件夹_局域网共享(WMI).ahk  
.13 新增 文件夹_局域网共享跳转.ahk  
.14 新增 旧版如意自定义动作配置转新版.ahk  
.15 新增 edge-chrome新标签页打开收藏.ahk  
.16 新增 结束指定进程并改名.ahk  
.17 新增 窗口移动到另一个屏幕.ahk  
10. 修复 Bug  
.01 修复 2.9 以来定时动作无法正常运行的 Bug  

### <span id="2.9">2.9 - 2025/07/12</span>
1. 修改 自动运行动作  
...
.07 新增 菜单模式使用字体图标=1 菜单模式中的图标使用 gdip 来绘制字体图标, 不使用 icon 图标  
3. 新增 本地帮助. 在动作管理界面新增动作的本地帮助按钮.  
4. 修改 动作管理窗口  
.01 新增 [动作内容编辑窗口](/actions/1703.md). 动作管理窗口动作内容旁新增一个文字按钮, 能打开动作内容编辑窗口, 方便编写动作的内容  
.02 修改 按钮动作图标设置改为 字符编码★颜色编号☆自定义字体名称@字体大小, 例如 5B57★FF0000☆宋体 表示宋体红色"字"  
.03 修改 动作说明中, 可以使用 \n 在面板按钮显示工具提示时换行  
5. 修改 面板和菜单  
...
.02 修复 2.9.0.1 引入的定时动作无法执行的 Bug  
...
3. 破坏性变动 自动运行动作管理界面, 使所有的自动运行动作管理使用同样的逻辑和管理界面, 定时动作配置使用新版的结构样式(旧版配置不再适用)  
4. 新增 如意内置函数和标签  
...
1. 破坏性变动 更改机器码的生成参数, 注册码单独保存. 旧注册码不再适用于新版, 付费用户请再次联系我免费获取新的注册码  

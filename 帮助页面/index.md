...
**当前版本**: [3.0 - 2025/08/15](history.md)
...
每个动作代表一个预设好的命令, 面板上的按钮对应一个动作, 点击面板上的按钮就会执行相应的动作. 动作设置保存在[\[1.1+\]](./history.md#1.1) "配置文件/内置动作.ini" 和 "配置文件/自定义动作.ini" 文件中.  
  ...
  -  动作除了通过面板或菜单运行外, 还可以通过以下方式运行  
     ...
     -  .8 [\[2.0+\]](./history.md#2.0)设置动作为[自动动作](./actions/1414.md), 启动如意时自动打开或定时自动打开  
     -  .9 [\[2.1+\]](./history.md#2.1)添加动作到[文件右键菜单](./actions/1342.md), 通过资源管理器中文件的右键菜单打开(启动方式同 6, 但是带有文件路径参数)  
     ...
     -  .13 [\[2.5+\]](./history.md#2.5)设置动作为[自动动作](./actions/1414.md), 监控剪贴板变化, 监控文件夹变化, 监控窗口变化, 满足条件时自动运行指定的动作
...
#### <span id="script">外部脚本</span>
  - 许多动作是由外部的 ahk 脚本来完成的, 它们统一放在 "外部脚本" 目录. 外部脚本在动作中最多支持 3 个命令行参数, 并且支持如意内置的[变量](./var.md). 现在内置动作自带的外部动作脚本有 315 个.  
    [完整的外部脚本列表](scriptlist.md)  
    下面列出了一些自带的外部脚本动作, 第二列为外部动作脚本是否集成在 AnyToAhk.exe(ATA) 中.  
